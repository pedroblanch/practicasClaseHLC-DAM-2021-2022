...
import { VerExpedientePage } from '../ver-expediente/ver-expediente.page';
...
export class HomePage implements OnInit {
  ...
  */
  eliminarAlumno(indice: number) {
    ...
    this.dataProvider.eliminarAlumno(this.alumnos[indice])
      .then(() => {
  ...
  */
  ...
  async verExpediente(indice: number) {
    const modal = await this.modalController.create({
    ...
    modal.onDidDismiss().then((dataAlumnoModificado) => {
      let alumnoModificado: Alumno = dataAlumnoModificado['data'];
      if (alumnoModificado != null) {
        this.alumnos[indice] = alumnoModificado;  //si se ha modificado se actualiza en la lista
      }
      else {
        //ha habido un error o se ha pulsado cancelar
        //no se hace nada
