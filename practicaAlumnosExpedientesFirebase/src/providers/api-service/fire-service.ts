...
export class FireServiceProvider implements InterfaceProvider {
    ...
    uploadPdfDocument(file: File, name: string): Promise<string> {
        var promise: Promise<string> = new Promise<string>((resolve, reject) => {
            ...
            const fileStoragePath = `expedientes/${name}`;
    ...
    */
    ...
    eliminarAlumno(alumno: Alumno): Promise<Boolean> {
        let promise = new Promise<Boolean>((resolve, reject) => {
            this.angularFirestore.collection('alumnos').doc(alumno.id).delete().then(
                (data: any) => { // Success
                    //el alumno se ha borrado correctamente en firestore database
                    //borro la imagen de avatar del alumno
                    this.removeFile(alumno.avatar)
                    //borro los documentos del expediente del alumno
                    alumno.expedientes.forEach(expediente=>{
                        this.removeFile(expediente.urlFile);
                    });
                    resolve(true);
                }
            )
